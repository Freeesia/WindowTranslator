...
public class HwndHostGroup
{
    ...
    private readonly HashSet<IHwndHolder> m_hwndHolders = new();
    private readonly PointDistanceComparer m_comparer = new();
    ...
    private void OnConnectorLoaded(object sender, RoutedEventArgs routedEventArgs) => ExpandHwndsAsync();

    private void OnConnectorUnloaded(object sender, RoutedEventArgs routedEventArgs) => CollapseHwnds(true);

    public void AddHost(IHwndHolder hwndHolder)
    {
        ArgumentNullException.ThrowIfNull(hwndHolder, nameof(hwndHolder));
    ...
    {
        if (m_hwndHolders.IsEmpty()) return;

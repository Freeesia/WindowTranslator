...
public sealed class OneOcr : IOcrModule, IDisposable
{
    ...
    public OneOcr(ILogger<OneOcr> logger, IOptionsSnapshot<LanguageOptions> langOptions, IOptionsSnapshot<BasicOcrParam> ocrParam)
    {
        ...
        {
            this.logger.LogWarning("環境変数 '{brightnessKey}' の値(-127 - 128) '{envBrightness}' は範囲外の為、明るさ調整は行われません。", brightnessKey, envBrightness);
            this.brightness = 0;
        }
        ...
        {
            this.logger.LogWarning("環境変数 '{contrastKey}' の値(-99 - 100) '{envContrast}' は範囲外の為、コントラスト調整は行われません。", contrastKey, envContrast);
            this.brightness = 0;
        }
    }
...
#pragma warning restore CA1416 // プラットフォームの互換性を検証
    ...
    public async ValueTask<IEnumerable<TextRect>> RecognizeAsync(SoftwareBitmap bitmap)
    {
        ...
        var adjustedBitmap = await AdjustBrightnessContrast(bitmap);
        // 拡大率に基づくリサイズ処理
        var workingBitmap = await adjustedBitmap.ResizeSoftwareBitmapAsync(this.scale);
        ...
        if (adjustedBitmap != workingBitmap)
        {
